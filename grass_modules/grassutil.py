...
class General(object):
    ...
    def list2dict(self, command, sep='=', sort=True, ):
    ...
    def grasslayerscheck(self, layers, type='raster', verbose=False):
    ...
    def listloc(self):
        return DictTable( 
        OrderedDict((i, 
        sorted([k for k in str(os.listdir(os.path.join(os.environ['GISDBASE'],i))).replace('[','').replace(']','').replace("'","").split(", ")]) 
...
class Raster(object):
    ...
    def univar(self, input, flags="ge", keys=None):
    ...
    def info(self, map, flags='r'):
        if General().grasslayercheck(map):
            return General().list2dict(grass.read_command('r.info', map=map, flags=flags), sep="=")
        else:
            print('input map %s not found' % input)
    ...
    def zoffset(self, map1, map2, offset):
        layers = [map1, map2]
        if all(General().grasslayercheck(layer) for layer in layers):
            grass.run_command('r.mapcalc' , expression = '%s = %s+abs(%s)' % (map1, map2, offset), overwrite=True)
        else:
            matchinglayers = [layer for layer in layers if General().grasslayercheck(layer)]
            # set(layers) & set(matchinglayers)
            missinglayers = len(layers) - len(matchinglayers)
    ...
    def width(self, grasslayer, report=False, plot=False):
        ...
        if plot:
            self._plot(zn[:,0], zn[:,3],
                      self.grasslayer+'_width_function.png',
